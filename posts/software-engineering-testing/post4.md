...
2.  **Symbolic State (Store):** This is a map of all variables to their current symbolic expressions (e.g., $x \to X_0 + 1$).
...
For example, if a path terminates with the PC $(X_0 > 10) \land (X_0 \cdot 2 = 30)$, the solver would return $X_0 = 15$. We have just automatically generated a test case, `x = 15`, that is guaranteed to execute that exact path.
