...
**Symbolic Execution** is a powerful method for generating tests that cover all program paths. However, it has practical limitations. It struggles when code involves complex math, external library calls, or dynamic features that a constraint solver cannot handle.
...
The concolic tester now adds `x = 3` to its queue. It re-runs the entire process with this new input, which is **guaranteed to take a different path** (the `else` branch). It will trace this new path, collect a new path condition ($X_0 \le 5$), and the process will terminate, as there are no more paths to find.
